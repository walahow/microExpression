...
# Use same constants, but import from LOCAL script
...
def run_cv():
    print(f"--- Starting {K_FOLDS}-Fold Subject-Independent Cross Validation (CelebA) ---")
    
    # 1. Load Data Availability
    all_files = glob.glob(os.path.join(FEATURES_DIR, "*.npy"))
    if len(all_files) == 0:
        print("No features found. Run train_lstm_celeba.py first to extract.")
        return
    ...
    print("\n\n=== CelebA Cross Validation Results ===")
